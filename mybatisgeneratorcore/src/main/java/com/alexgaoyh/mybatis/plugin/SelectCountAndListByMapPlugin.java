...
public class SelectCountAndListByMapPlugin extends PluginAdapter {
	...
	public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		String orderBySqlJudge = "<if test=\"orderBySqlJudgeStr != null and orderBySqlJudgeStr != ''\"> order by ${orderBySqlJudgeStr} </if>"
				+ "<if test=\"orderBySqlJudgeStr == null or orderBySqlJudgeStr == ''\"> order by " + orderByStr.toString() + "</if>";
		...
		// alexgaoyh begin notEqual != 不等部分的 操作
		StringBuilder sbNotEqual = new StringBuilder();
		for (IntrospectedColumn introspectedColumn : introspectedTable
                .getAllColumns()) {
			XmlElement valuesNotNullElement = new XmlElement("if"); //$NON-NLS-1$
			sbNotEqual.setLength(0);
			sbNotEqual.append("myNotEqual_" + introspectedColumn.getJavaProperty());
			sbNotEqual.append(" != null"); //$NON-NLS-1$
			sbNotEqual.append(" and "); //$NON-NLS-1$
			sbNotEqual.append("myNotEqual_" + introspectedColumn.getJavaProperty());
			sbNotEqual.append(" != ''"); //$NON-NLS-1$
            valuesNotNullElement.addAttribute(new Attribute(
                    "test", sbNotEqual.toString())); //$NON-NLS-1$

            sbNotEqual.setLength(0);
            sbNotEqual.append(" AND ");
            sbNotEqual.append(MyBatis3FormattingUtilities
                    .getAliasedEscapedColumnName(introspectedColumn));
            sbNotEqual.append(" != "); //$NON-NLS-1$
            sbNotEqual.append(MyBatis3FormattingUtilities
                    .getParameterClause(introspectedColumn, "myNotEqual_"));
            valuesNotNullElement.addElement(new TextElement(sbNotEqual.toString()));
            trimElement.addElement(valuesNotNullElement);
		}
		// alexgaoyh end notEqual != 不等部分的 操作
